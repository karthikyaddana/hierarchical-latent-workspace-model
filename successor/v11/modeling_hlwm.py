...
class HLWMConfig:
    ...
    # Version 11.0 vocabulary-grounded thoughts: each thought embedding is
    # constructed INSIDE the decoder's token basis (state -> frozen lm_head ->
    # temperature softmax -> coefficient-weighted average of frozen input
    # embeddings), so the write cannot leave the vocabulary-indexed subspace.
    # Motivated by the v10.0 J-space verdict (content written to decoder-inert
    # directions) and by J-CoT (arXiv:2607.21981). Zero-cost when False.
...
class HLWMForConditionalGeneration(nn.Module):
    ...
    def produce_latent_thoughts(
        ...
        for index in range(self.config.latent_thoughts):
            ...
            if self.config.vocab_grounded_thoughts:
                # Version 11.0: decode the projected thought through the FROZEN
                # unembedding and rebuild it as a coefficient-weighted average
                # of FROZEN input embeddings.  The thought therefore lives in
                # the span of token embeddings with vocabulary-indexed
                # coordinates the decoder's own output map can read -- the
                # write is aligned by construction, not by hope (the v10.0
                # failure measured in the J-space audit).  Softmax stays dense
                # for differentiability; the top coefficients are the channel's
                # human-readable transcript.
                vocab_logits = self.lm_head(
                    projected.to(self.lm_head.weight.dtype)
                ).float()
                coefficients = torch.softmax(
                    vocab_logits / max(self.config.vocab_thought_tau, 1.0e-4),
                    dim=-1,
                )
                projected = (
                    coefficients.to(self.backbone.embed_tokens.weight.dtype)
                    @ self.backbone.embed_tokens.weight
                ).to(hidden.dtype)
