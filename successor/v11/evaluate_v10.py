...
def arm_declaration_routed(
    model: HLWMForConditionalGeneration,
    prepared: Sequence[PreparedRow],
    tokenizer: Any,
    *,
    max_new_tokens: int = 48,
    declaration_tokens: int = 12,
) -> Dict[str, Any]:
    """(i) Declaration-routed: the route comes from the model's own tokens.

    Two phases, and the split is the point. Phase one decodes at most
    ``declaration_tokens`` through the always-on root path with NO expert
    applied, so nothing about that emission depends on the route it selects.
    The declaration is parsed out of that text; phase two decodes the answer
    with the declared expert applied.

    This is what arms (a)-(h) cannot offer. Gold routing (a) uses a label the
    deployment path does not have, and the probe router (h) reads a hidden
    state, so in both the route is an inference ABOUT the model rather than an
    act BY it. Here the tokens that select the expert are in the output stream,
    which makes causal liveness something you read off a transcript instead of
    something you infer from gate statistics -- the measurement this record
    failed to obtain on 10 of 10 seed-runs.

    Undeclared and unknown-name rows fall back to the root path (route None)
    and are counted separately; which of those two the primary gate tolerates
    is a preregistered decision, not a default set here.
    """

    routes = list(FAMILY_ORDER)
    expert_count = int(getattr(model.config, "mlp_expert_count", 0))
    router_families = int(getattr(model.config, "router_families", 4))
    decode_ids: List[Tensor] = []
    declared_families: List[Optional[int]] = []
    declared_routes: List[Optional[int]] = []
    declaration_texts: List[str] = []
    undeclared = 0
    unknown = 0
    with torch.no_grad():
        for item in prepared:
            preamble = _decode_v10_row(
                model, item, item.thought_embeds, item.thought_states,
                use_prefix_slots=True, route_index=None,
                max_new_tokens=int(declaration_tokens),
            )
            text = tokenizer.decode(preamble[0].tolist(), skip_special_tokens=True)
            declaration_texts.append(text)
            family = parse_declaration(text, routes)
            declared_families.append(family)
            if family is None:
                # Distinguish "emitted nothing parseable" from "emitted a
                # declaration naming a route that does not exist": they are
                # different failure modes and collapsing them would hide one.
                ...
                route = route_for_family_index(family, expert_count, router_families)
            declared_routes.append(route)
            decode_ids.append(
                _decode_v10_row(
                    model, item, item.thought_embeds, item.thought_states,
                    use_prefix_slots=True, route_index=route,
                    max_new_tokens=max_new_tokens,
                )
            )
    total = len(prepared)
    agree = sum(
        1
        for item, family in zip(prepared, declared_families)
        if family is not None and int(family) == int(item.family_index)
    )
    return {
        "arm": "declaration_routed",
        "decode_ids": decode_ids,
        ...
        "declaration_audit": {
            "total": total,
            "declared": total - undeclared - unknown,
            "declared_fraction": (total - undeclared - unknown) / total if total else 0.0,
            "undeclared": undeclared,
            "unknown": unknown,
            "gold_agreement": agree / total if total else 0.0,
        },
    }
...
def grade_decode(
    ...
    # Route declarations are harness control tokens, never answer content.
    # Stripping at the single grading choke point keeps a declared-routing run
    # graded on exactly the surface every other arm is graded on; with the
    # flag off this is a no-op substitution.
    text = strip_declarations(text)
