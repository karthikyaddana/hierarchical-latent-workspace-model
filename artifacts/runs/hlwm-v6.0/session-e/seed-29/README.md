# HLWM Version 6.0 - verified fan-in

Version 6.0 keeps the workspace-only Version 5.7 base and adds the three mechanisms aimed at the program's one open quality hypothesis, each with its own causal test or killer baseline in the same audit:

1. **Latent read-out memory** (`workspace_memory_windows 16`): the synthesis prefix widens from a linear reshape of three pooled vectors to 43 structured latent tokens (16 windowed canvas states + 1 summary per lane), zero-init projected. Hidden states cross the barrier, never token identities. The audit re-decodes every row with the memory sliced off and reports the causal delta.
2. **Verified fan-in** (temperatures 0.0/0.7/0.9/1.1): one workspace, four candidates, calibrated head selection, publication gated by the fitted rule. Baselines on the same rows and budget: self-consistency majority vote and training-free max-mean-logprob selection over four causal-path samples.
3. **Scalar publication rule**: regularized logistic combiner over the de-correlated head probabilities, one threshold, midpoint-of-margin selection - the structural fix for six degenerate-threshold recurrences. A coverage-matched mean-logprob abstention baseline is reported.

Trainable parameters 76,627,079 (+1.9% vs Study 6's 75,174,348; the plain-LoRA control is budget-matched at its own rank when it runs). 20-gate battery = 15 Version 5.7 gates + 5 fan-in gates. Plan: `reports/hlwm-v6.0-plan-2026-09-01.md`.
