# HLWM Kaggle Version 5.2

Version 5.2 is a correctness-first production candidate built around the pinned
...
Version 5.1 reached the real Qwen load on both T4s but its first local loss was
non-finite. Version 5.2 therefore:
...
- keeps every transplanted Qwen tensor frozen in FP16 and tunes FP32 rank-8
  LoRA sidecars in the last four decoder blocks;
- computes 151,936-way softmax and token losses in FP32 and excludes padded
  rows before cross-entropy;
- uses exact boolean attention masks and forces padding states back to zero;
- skips the unused reverse posterior and dense one-hot embedding multiply in
  one-pass local training;
- runs a named real-Qwen forward/backward finiteness preflight at the exact
  production token lengths before the dual launch;
...
seeds, long-run reliability and deployment load tests. Version 5.2 is designed to
